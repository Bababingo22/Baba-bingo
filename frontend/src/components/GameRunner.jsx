...
const getBingoLetter = (number) => { /* ... (This function is correct, no changes needed) ... */ };
const CardCheckModal = ({ cardData, calledNumbers, onClose }) => { /* ... (This component is correct, no changes needed) ... */ };
    ...
    <div className="bg-[#1e2b3a] p-4 rounded-lg flex-1">
      ...
      <div className="grid grid-cols-16 gap-1 text-center">
        ...
        {Array.from({ length: 15 }, (_, i) => i + 1).map(headerNum => (
          <div key={`header-${headerNum}`} className="font-bold text-gray-400">{headerNum}</div>
        ))}
          ...
          <React.Fragment key={letter}>
            ...
            {Array.from({ length: 15 }, (_, colIndex) => {
              ...
              return (
                <div 
                  key={num} 
                  className={`w-10 h-10 flex items-center justify-center text-md font-semibold rounded-full transition-colors ${
