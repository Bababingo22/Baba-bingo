...
// ... (All the helper functions at the top like getBingoLetter, CardCheckModal, NumberGrid, etc. are the same) ...
...
const CardCheckModal = ({ checkResult, calledNumbers, onClose }) => {
  ...
  const isWinner = parseBooleanLike(checkResult.is_winner) || parseBooleanLike(checkResult.winner) || parseBooleanLike(checkResult.card_data?.is_winner) || (typeof checkResult.result === 'string' && checkResult.result.toLowerCase().includes('win'));
  ...
  const handleBad = () => { playAudio('/audio/male/bad.mp3'); onClose(); };
  const handleGood = () => { playAudio('/audio/male/good.mp3'); onClose(); };
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" role="dialog" aria-modal="true" aria-label="Card check result">
      <div className="bg-[#1f2937] p-6 rounded-lg shadow-2xl w-full max-w-2xl">
        <header className="flex items-center justify-between mb-4">
          <div><h2 className="text-xl font-semibold text-white">Vlad Bingo</h2><p className="text-sm text-gray-300">Card Number: <span className="font-medium">{card_number}</span></p></div>
          <button onClick={onClose} aria-label="Close" className="text-gray-300 hover:text-white">✕</button>
        </header>
        <div className={`p-3 rounded-md mb-4 ${isWinner ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}><h3 className="text-center text-2xl font-bold">{isWinner ? 'ዘግቷል' : 'አልዘጋም'}</h3></div>
        <div className="overflow-auto">
          <table className="w-full border-separate" style={{ borderSpacing: '6px' }}>
            <thead><tr>{headers.map((h, i) => (<th key={h} className={`w-1/5 text-center text-lg font-semibold p-2 text-white rounded-md ${colors[i]}`}>{h}</th>))}</tr></thead>
            <tbody>
              {rows.map((row, rowIndex) => (<tr key={rowIndex}>{row.map((cellValue, colIndex) => {
                    const isFreeSpace = cellValue === "FREE";
                    const called = isCellCalled(cellValue);
                    const cellClass = called ? 'bg-transparent text-yellow-400 font-bold shadow-[0_0_12px_rgba(250,204,21,0.9)] scale-105 transition-all duration-200' : isFreeSpace ? 'bg-blue-600 text-white' : 'bg-gray-200 text-black';
...
const NumberGrid = ({ calledNumbers }) => {
  ...
  return (
    <section className="bg-[#0b1220] p-4 rounded-lg shadow-inner h-full min-h-[260px] flex flex-col">
      <div className="mb-3 flex items-center justify-between"><h4 className="text-lg font-semibold text-white">Called Numbers</h4><span className="text-sm text-gray-300">Grid 1–75</span></div>
      <div className="flex-1 overflow-auto">
        <table className="w-full border-separate" style={{ borderSpacing: '6px' }}>
          <thead><tr>{headers.map((letter, index) => (<th key={letter} className={`text-white font-semibold text-lg text-center rounded-md ${headerColors[index]} py-2`}>{letter}</th>))}</tr></thead>
          <tbody>
            {Array.from({ length: 15 }).map((_, rowIndex) => (<tr key={rowIndex}>{Array.from({ length: 5 }).map((_, colIndex) => {
                  const num = colIndex * 15 + rowIndex + 1;
                  const isCalled = calledNumbers.has(num);
                  const cellClass = isCalled ? 'bg-transparent text-yellow-400 font-bold shadow-[0_0_12px_rgba(250,204,21,0.9)] transform scale-105 transition-all duration-200' : 'bg-transparent text-gray-300';
                  return (<td key={num} className={`text-center font-semibold text-base py-2 rounded ${cellClass}`} aria-pressed={isCalled}>{num}</td>);
                })}</tr>))}
          </tbody>
        </table>
      </div>
...
export default function GameRunner({ game, token, user, callSpeed, audioLanguage, onNav }) {
  ...
  const prizeAmount = (() => {
    ...
    // It now uses the game's commission, not the user's default commission.
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-[#0f172a] p-4 rounded-lg shadow-sm flex flex-col md:flex-row items-center md:items-stretch gap-4">
                ...
                <div className="flex-1 flex flex-col gap-3">
                  <button onClick={() => setIsPaused(prev => !prev)} className={`w-full py-3 rounded-md font-semibold ${isPaused ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-orange-500 hover:bg-orange-600'} transition-colors`} aria-pressed={!isPaused}>
                    {isPaused ? 'Resume Calls' : 'Pause Calls'}
                  </button>
                  <div className="flex gap-2">
                    <label htmlFor="card-check" className="sr-only">Card Number</label>
                    <input id="card-check" type="number" placeholder="Card #" value={cardNumberToCheck} onChange={(e) => setCardNumberToCheck(e.target.value)} className="flex-1 bg-[#071122] border border-gray-700 rounded-md px-3 py-2 placeholder:text-gray-400 text-white"/>
