...
const rows = Array.from({ length: 5 }).map((, r) => Array.from({ length: 5 }, (, c) => board[c][r]));
...
<tbody>
{Array.from({ length: 15 }).map((, rowIndex) => (
<tr key={rowIndex}>
{Array.from({ length: 5 }).map((, colIndex) => {
const num = colIndex * 15 + rowIndex + 1;
const isCalled = calledNumbers.has(num);
return (
<td
key={num}
className={text-center font-semibold text-lg transition-colors duration-300 ${ isCalled ? 'text-yellow-400 font-bold' : 'text-gray-600' }}
>
{num}
</td>
);
