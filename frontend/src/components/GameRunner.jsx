    ...
    <div className="flex-1 bg-[#1e293b] p-4 rounded-lg">
      <div className="flex justify-around">
        {headers.map((header, colIndex) => (
          <div key={header} className="flex flex-col items-center gap-2">
            <div className="w-12 h-12 flex items-center justify-center text-3xl font-bold">{header}</div>
            {columns[colIndex].map(num => (
              <div key={num} className={`w-12 h-12 flex items-center justify-center text-xl font-semibold rounded-full transition-colors ${calledNumbers.has(num) ? 'bg-yellow-400 text-black' : 'bg-gray-700 text-gray-300'}`}>
                {num}
              </div>
            ))}
          </div>
    ...
    <>
      ...
      <div className="flex bg-[#0f172a] text-white p-6 gap-6 min-h-screen">
        <div className="w-64 flex flex-col gap-6">
          <div className="bg-[#1e293b] p-4 rounded-lg text-center"><div className="text-gray-400 font-semibold">Total Calls</div><div className="text-7xl font-bold">{calledNumbers.size}</div></div>
          <div className="bg-[#1e293b] p-4 rounded-lg text-center">
            <div className="text-gray-400 font-semibold mb-2">Winning Pattern</div>
            <div className="grid grid-cols-5 gap-1 mx-auto w-40 h-40 border-2 border-gray-600 p-1">{Array.from({length: 25}).map((_, i) => <div key={i} className={`rounded-full ${[0,6,12,18,24].includes(i) ? 'bg-yellow-400' : 'bg-blue-800'}`}></div>)}</div>
          </div>
          <div className="bg-[#1e293b] p-4 rounded-lg text-center flex-1"><div className="text-gray-400 font-semibold">Next Number</div><div className="text-8xl font-bold mt-4">{nextNumber || '-'}</div></div>
