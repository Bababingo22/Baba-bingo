...
export default function GameRunner({ game, token, user, callSpeed, audioLanguage, onNav }) {
  ...
  // --- THIS IS THE FIX: Initialize state from the game prop ---
  const initialCalls = game.called_numbers || [];
  const [currentNumber, setCurrentNumber] = useState(initialCalls.length > 0 ? initialCalls[initialCalls.length - 1] : null);
  const [callHistory, setCallHistory] = useState(initialCalls.length > 1 ? initialCalls.slice(0, -1).reverse() : []);
    ...
    socketRef.current.onmessage = (ev) => {
      ...
      if (data.action === "call_number") {
        ...
        setCurrentNumber(prev => {
          if (prev) { setCallHistory(h => [prev, ...h].slice(0, 4)); }
          return newNumber;
        });
        speakText(newNumber, audioLanguage);
        setCountdown(callSpeed);
      }
    };

    socketRef.current.onopen = () => {
      // --- THIS IS THE FIX: Only announce "Game Started" if it's a brand new game ---
  ...
  function speakText(textOrNumber, lang, isAnnouncement = false) {
    ...
    let textToSpeak = isAnnouncement ? textOrNumber : `${getBingoLetter(textOrNumber)} ${textOrNumber}`;
